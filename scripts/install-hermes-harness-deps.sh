...
docker exec "$HERMES_CONTAINER" sh -lc '
  ...
  python3 -m pip install --break-system-packages firecrawl-py tavily-python exa-py
  if [ -x /opt/hermes/.venv/bin/python ]; then
    ...
    /opt/hermes/.venv/bin/python -m pip install firecrawl-py tavily-python exa-py
  fi
  python3 - <<PY
import sys
...
print("tavily_client import ok:", getattr(tavily_client, "__file__", ""))
print("exa_client import ok:", getattr(exa_client, "__file__", ""))
PY
  if [ -x /opt/hermes/.venv/bin/python ]; then
...
PY
  ...
  python3 /opt/data/workspace/report-agent/skills/web-research-firecrawl/scripts/research_cli.py --help >/dev/null
