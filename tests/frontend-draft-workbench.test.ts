...
assert.ok(analysisSource.indexOf('返回重新输入') < analysisSource.indexOf('生成提纲'));
assert.equal((analysisSource.match(/返回重新输入/g) || []).length, 1);
assert.equal((analysisSource.match(/生成提纲/g) || []).length, 1);
assert.match(analysisSource, /\.draft-analysis-actions\s*\{[^}]*position:\s*fixed/);
